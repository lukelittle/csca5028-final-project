...
class TestQueueService(unittest.TestCase):
    def setUp(self):
        ...
        self.queue_service = QueueService(self.mq_client_mock, "test-queue")

    def test_publish_message(self):
        test_message = {"key": "value"}
        self.queue_service.publish_message(test_message)
        self.mq_client_mock.queue.assert_called_with("test-queue")
        self.mq_client_mock.queue("test-queue").post.assert_called_with(test_message)

    def test_get_messages(self):
        self.queue_service.get_messages()
        self.mq_client_mock.queue.assert_called_with("test-queue")
        self.mq_client_mock.queue("test-queue").get.assert_called()

    def test_delete_message(self):
        test_message_id = "123"
        self.queue_service.delete_message(test_message_id)
        self.mq_client_mock.queue.assert_called_with("test-queue")
        self.mq_client_mock.queue("test-queue").delete.assert_called_with(test_message_id)
